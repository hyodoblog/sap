  ...
  def on
    if current_user.login_info_email_flag
      ...
      laboratories.each do |laboratory|
        Notification.send_login_info(laboratory, current_user).deliver unless laboratory.email.empty?
      end
      students.each do |student|
        Notification.send_login_info(studnet, current_user).deliver unless student.email.empty?
      end
    end
